...
class Pdate
{
	...
	public static function pdate($format, $timestamp = NULL)
	{
        ...
        while($i < $lenghFormat)
        {
            ...
            switch($par)
            {
                ...
                case 'D':
                    $result .= substr(Pdate::$pdate_week_name[$pWeek], 0, 2);
                    break;
                ...
                case 'l':
                    $result .= Pdate::$pdate_week_name[$pWeek];
                    break;
                ...
                case 'z':
                    $result .= Pdate::DayOfYear($pMonth, $pDay);
                    break;
                ...
                # Week
                case 'W':
                    $result .= ceil(Pdate::DayOfYear($pMonth, $pDay) / 7);
                    break;

                # Month
                case 'F':
                    $result .= Pdate::$pdate_month_name[$pMonth];
                    break;
                ...
                case 'M':
                    $result .= substr(Pdate::$pdate_month_name[$pMonth], 0, 6);
                    break;
                ...
                case 't':
                    $result .= ((Pdate::isKabise($pYear) && ($pMonth == 12)) ? 30 : Pdate::$pdate_month_days[$pMonth]);
                    break;

                # Years
                case 'L':
                    $result .= (int)Pdate::isKabise($pYear);
                    break;
                ...
                case 'r':
                    $result .= (substr(Pdate::$pdate_week_name[$pWeek], 0, 2) . '، ' . $pDay . ' ' . substr(Pdate::$pdate_month_name[$pMonth], 0, 6) . ' ' . $pYear . ' ' . date('H::i:s P', $timestamp));
                    break;
	...
	public static function pstrftime($format, $timestamp = NULL)
	{
		//global $pdate_month_name, $pdate_week_name, $pdate_month_days;
        if(!$timestamp)
        {
        ...
        while($i < $lenghFormat)
        {
            ...
            if($par == '%')
            {
                ...
                switch($type)
                {
                    ...
                    case 'j':
                        $dayinM = Pdate::DayOfYear($pMonth, $pDay);
                    ...
                    # Week
                    case 'U':
                        $result .= floor(Pdate::DayOfYear($pMonth, $pDay) / 7);
                        break;
                    ...
                    case 'c':
                        $result .= (substr(Pdate::$pdate_week_name[$pWeek], 0, 2) . ' ' . substr(Pdate::$pdate_month_name[$pMonth], 0, 6) . ' ' . $pDay . ' ' . strftime("%T", $timestamp) . ' ' . $pYear);
                        break;
	...
	public static function pcheckdate($month, $day, $year)
	{
        ...
        if($day > Pdate::$pdate_month_days[$month])
        {
            if(($month != 12) || ($day != 30) || !Pdate::isKabise($year))
            {
                return FALSE;
            }
        }
	...
	public static function pgetdate($timestamp = NULL)
    {
        if(!$timestamp)
        {
            $timestamp = mktime();
